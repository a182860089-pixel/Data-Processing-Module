...
## 附录 A：微信公众号文章爬虫功能（已实现）

### A.1 功能概述
基于 crawl4ai 实现的微信公众号文章爬虫模块，能够自动爬取微信公众号文章的正文内容，并将其转换为 Markdown 格式。

### A.2 技术选型
- **爬虫框架**: crawl4ai (v0.3.7+)
- **浏览器引擎**: Chromium (通过 Playwright)
- **内容处理**: PruningContentFilter + DefaultMarkdownGenerator
...
BrowserConfig(
    ...
    viewport_width=1280,
    viewport_height=800,
    user_agent="Chrome/120.0.0.0",    # 模拟Chrome浏览器
    extra_args=[
...
CrawlerRunConfig(
    ...
    page_timeout=30000,               # 30秒页面超时
    delay_before_return_html=2.0,     # 等待JS渲染
...
#### A.4.3 内容处理流程
1. 验证URL格式 (mp.weixin.qq.com)
2. 启动Chromium浏览器访问页面
3. 等待页面JS渲染完成
4. 提取页面HTML内容
5. 使用PruningContentFilter过滤噪音内容
6. 使用DefaultMarkdownGenerator转换为Markdown
7. 清理微信特有的干扰内容
8. 返回标题和正文
...
async def crawl_article():
    async with httpx.AsyncClient() as client:
        response = await client.post(
            "http://localhost:8000/api/v1/crawl/wechat",
            json={
...
#### cURL调用示例
```bash
curl -X POST "http://localhost:8000/api/v1/crawl/wechat" \
...
| 文件路径 | 说明 |
|---------|------|
| `app/services/crawler/__init__.py` | 爬虫模块初始化 |
| `app/services/crawler/wechat_crawler.py` | 微信爬虫核心实现 |
| `app/api/v1/endpoints/crawler.py` | API接口定义 |
| `requirements.txt` | 添加 crawl4ai 依赖 |
| `app/main.py` | 注册爬虫路由 |
